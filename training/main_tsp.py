...
def is_tsptw(config):
    return config.get("problem", "tsp") == "tsptw"

def get_data_reader(config):
    return TSPTWReader if is_tsptw(config) else TSPReader
...
def train_one_epoch(net, optimizer, config, master_bar, dataset=None):
    ...
    if dataset is None:
        dataset = get_data_reader(config)(
            num_nodes,
            num_neighbors,
            batch_size,
            train_filepath,
            config.train_filepath_solution,
            do_shuffle=True,
            do_prep=False,
        )
    else:
    ...
    for batch_num in progress_bar(range(batches_per_epoch), parent=master_bar):
        ...
        x_nodes_coord = Variable(
            torch.FloatTensor(batch.nodes_coord).type(dtypeFloat), requires_grad=False
        )
        x_nodes_timew = (
            Variable(
                torch.FloatTensor(batch.nodes_timew).type(dtypeFloat),
                requires_grad=False,
            )
            if is_tsptw(config)
            else None
        ...
        y_tour = Variable(
            torch.LongTensor(batch.tour_nodes).type(dtypeLong), requires_grad=False
        )
        ...
        y_preds, loss, x_edges_values = net.forward(
            x_nodes_coord, x_nodes_timew, y_tour, edge_cw
        )
        ...
        running_loss += (
            batch_size * loss.data.item() * accumulation_steps
        )  # Re-scale loss
...
def test(net, config, master_bar, mode="test"):
    ...
    DataReader = get_data_reader(config)
    ...
    if mode == "val":
        dataset = DataReader(
            ...
            target_filepath=val_target_filepath,
            do_prep=False,
        )
    elif mode == "test":
        dataset = DataReader(
            ...
            target_filepath=test_target_filepath,
            do_prep=False,
        )
    batches_per_epoch = dataset.max_iter
    ...
    with torch.no_grad():
        ...
        for batch_num in progress_bar(range(batches_per_epoch), parent=master_bar):
            ...
            # x_nodes = Variable(torch.LongTensor(batch.nodes).type(dtypeLong), requires_grad=False)
            x_nodes_coord = Variable(
                torch.FloatTensor(batch.nodes_coord).type(dtypeFloat),
                requires_grad=False,
            )
            x_nodes_timew = (
                Variable(
            ...
            if type(edge_cw) != torch.Tensor:
                ...
                num_edge_classes = 2
                # Don't make tensor since then it will mess up DataParallel, this is a parameter, not input!
                edge_label_bincount = np.array(
                    [num_edges - 2 * num_nodes, 2 * num_nodes]
                )
                edge_cw = num_edges / (num_edge_classes * edge_label_bincount)
...
def train(config, dataset, solutions=None):
    ...
    if "resume_from_dir" in config:
        if torch.cuda.is_available():
            checkpoint = torch.load(
                os.path.join(config.resume_from_dir, "last_train_checkpoint.tar")
            )
        else:
            checkpoint = torch.load(
                os.path.join(config.resume_from_dir, "last_train_checkpoint.tar"),
    ...
    for epoch in epoch_bar:
        ...
        (
            ...
            train_pred_tour_len,
            train_gt_tour_len,
        ) = train_one_epoch(net, optimizer, config, epoch_bar, dataset=dataset)
        epoch_bar.write(
            "t: "
            + metrics_to_str(
                epoch,
        ...
        writer.add_scalar(
            "pred_tour_len/train_pred_tour_len", train_pred_tour_len, epoch
        )
        writer.add_scalar(
            "optimality_gap/train_opt_gap",
            train_pred_tour_len / train_gt_tour_len - 1,
            epoch,
        )

        if val_every > -1 and (epoch % val_every == 0 or epoch == max_epochs - 1):
            # Validate
            (
                val_time,
                val_loss,
            ...
            writer.add_scalar(
                "pred_tour_len/val_pred_tour_len", val_pred_tour_len, epoch
            )
            writer.add_scalar(
                "optimality_gap/val_opt_gap",
                val_pred_tour_len / val_gt_tour_len - 1,
                epoch,
            )
            ...
            if val_pred_tour_len < best_pred_tour_len:
                best_pred_tour_len = (
                    val_pred_tour_len  # Update best val predicted tour length
                )
                torch.save(
                    {
                        "epoch": epoch,
        ...
        if test_every > -1 and (epoch % test_every == 0 or epoch == max_epochs - 1):
            # Test
            (
                test_time,
                test_loss,
            ...
            writer.add_scalar(
                "pred_tour_len/test_pred_tour_len", test_pred_tour_len, epoch
            )
            writer.add_scalar(
                "optimality_gap/test_opt_gap",
                test_pred_tour_len / test_gt_tour_len - 1,
                epoch,
            )
